...
const CategoryProducts = () => {
    ...
    const fetchProductsByCategory = async () => {
      try {
        const res = await axios.get(
          `${import.meta.env.VITE_API_URL}/api/get-products-list?category=${categoryId}&status=active&gender=${gender}&search=${search}`,
        );
        if (res.data.success) {
        ...
        toast.error(
          error?.response?.data?.message ||
            "Unable to get products of this category",
        );
      }
    };
    fetchProductsByCategory();
  }, [gender, search, categoryId]);
  ...
  return (
    <div className="min-h-screen bg-[linear-gradient(to_bottom_right,#fdf2f8,#ffffff,#fce7f3)] p-8">
      ...
      {/* Product Grid */}
      <div
        className="max-w-7xl mx-auto grid gap-4 mt-10 
